...
import argparse, csv, collections, ConfigParser, urllib, json, requests, random, ujson, os, time
...
def get_photo(lat, lon):
    ...
    if there is no photo there (or OVER_QUERY_LIMIT if we're rate limited"""
    ...
    try:
        ...
        if metadata_resp['status'] == 'OK':
            ...
            print "zero results"
            return 'ZERO_RESULTS'
        elif metadata_resp['status'] == 'OVER_QUERY_LIMIT':
            print "Over query limit, sleeping for 1 hr"
            time.sleep(60*60)
...
if __name__=='__main__':
    ...
    for nghd, latlons in nghd_latlons.iteritems():
        ...
        for i in range(args.photos_per_nghd * 5): # try up to 50 times
            if os.path.isfile(args.output_dir + '%s_%s.png' % (nghd, found)):
                print "Already exists: ", args.output_dir + '%s_%s.png' % (nghd, found)
                # If we've already got photos for a neighborhood, no sense
                # getting them again.
                found += 1
            ...
            if photo == 'ZERO_RESULTS' or photo == 'OVER_QUERY_LIMIT':
                pass
            else:
